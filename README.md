...
- [ ] Layout Change
  - Panel settings device name ip mapping
    - Use similar ui to the machine list of tailscale
      - Machine name, combined addresses into a single cell, date last seen, owner
      - Individual edit and delete buttons per machine
      - Pressing edit expands the row of that machine to show all editable values and allow editing
      - When in edit mode, the edit button turns into a save button and the delete button turns into a cancel button.
  - Global
    - Split the textinput box class into 2 - title row input and content area/ card input
    - Title row input retains the current color scheme
    - Content area/ card input now uses the same color as the pane class background color (light and dark mode)
Known bugs:
  ...
  - We have different incorrect password modals right now that need to be merged into a single incorrect password modal that still shows the image and the password input box, no more error code for incorrect password:
    - Homepage:
        Action Rejected
        Incorrect password image
        Password incorrect. Whatever you were trying to do is cancelled.
    - Cleanup page:
    ...
    - Panel Settings page
        Panel Settings
        Enter superadmin password to continue.

        •
        Password incorrect.
  - Password retry cooldown still shows at page reload even after the cooldown ended.
    Notice
    Password retries paused for 9 seconds after 3 failed attempts.
